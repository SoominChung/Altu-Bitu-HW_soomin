...
string haligarli(int m, deque<int> do_deck, deque<int> su_deck, queue<int> do_ground, queue<int> su_ground){
    ...
    while(cnt < m){
        ...
        cnt++; //do가 카드 냈으므로 라운드 증가

        // do가 카드 냈으니 종치는 조건 만족하는지 확인
        // 1. 수연이 종치는 조건
        if(!su_ground.empty() && !do_ground.empty()) {
            if(do_ground.back()+su_ground.back() == 5) { //수정: do_ground도 back을 하려면 empty()아닌지 확인해야함. (문제 조건(수 ground 비어있으면 안된다는)만 보다가 당연한건데 놓쳤음)
                for (int i = 0; i < do_ground.size(); i++) { //do의 ground deck 뒤집어서 수연이가 가져감
        ...
        // 2. do의 종치는 조건
        if(!su_ground.empty() && !do_ground.empty()) { //수정: empty 확인 안했었음. 값에 접근할 때는 empty 확인 필수!
        ...
        su_deck.pop_back();
        cnt++; //su가 카드 냈으므로 라운드 증가

        // su가 카드 냈으니 종치는 조건 만족하는지 확인
        // 1. 수연이 종치는 조건
        if(!su_ground.empty() && !do_ground.empty()) {
            if(do_ground.back()+su_ground.back() == 5) { //수정: do_ground도 back을 하려면 empty()아닌지 확인해야함. (문제 조건(수 ground 비어있으면 안된다는)만 보다가 당연한건데 놓쳤음)
                for (int i = 0; i < do_ground.size(); i++) { //do의 ground deck 뒤집어서 수연이가 가져감
